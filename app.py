...
def pay_bill(user, bill_type, amount, account_number):
    ...
    record_transaction(
        user,
        f"Bill Payment: {bill_type} | Acct: {account_number} | ${amount} ({timestamp})"
    )
...
menu = st.selectbox(
    "Menu",
    ["Login", "Register"] if st.session_state["user"] is None else ["Wallet", "Logout"]
)
...
# --------------------------
    ...
    bill_type = st.selectbox(
        "Select Bill",
        ["T&TEC Electricity", "TSTT", "Digicel", "FLOW", "WASA", "Internet"]
    )

    account_number = st.text_input("Account Number")

    bill_amount = st.number_input("Bill Amount (TTD)", min_value=1.0, step=1.0)

    if st.button("Pay Bill"):
        if account_number.strip() == "":
            st.error("Account number is required.")
        else:
            ok, msg = pay_bill(user, bill_type, bill_amount, account_number)
            if ok:
                st.success(msg)
                st.rerun()
            else:
    ...
    bank = st.selectbox(
        "Select Bank",
        ["Republic Bank", "RBC Royal Bank", "Scotiabank", "First Citizens (FCB)", "JMMB"]
    )
